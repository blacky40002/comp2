...
def extract_features(text, config):
    ...
    # Normalizzazione basata sul numero di parole o caratteri
...
def run(base_path):
    ...
    results = []
    print(f"\n Validazione {len(configs)} configurazioni")
    ...
    try:
        ...
        # Feature Importance per ogni autore
        feature_names = final_vec.get_feature_names_out()
        for idx, author_name in enumerate(final_svm.classes_):
            utils_plot.plot_feature_importance(
                feature_names,
                final_svm.coef_[idx],
                title=f"Task 3 - Top N-grammi ({author_name})",
                filename=f"03_feature_importance_{author_name}.png",
                top_n=20
            )
    except Exception as e:
